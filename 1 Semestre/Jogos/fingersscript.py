...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        ...
        # Verifica quantas mãos estão fechadas e digita a letra correspondente
        if hands_down_count == 1 and not previous_one_hand_down:
            pyautogui.write('a')
            cv2.putText(frame, 'A', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
            previous_one_hand_down = True
            previous_two_hands_down = False  # Reset two hands down state
        elif hands_down_count == 2 and not previous_two_hands_down:
            pyautogui.write('b')
            cv2.putText(frame, 'B', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
            previous_two_hands_down = True
            previous_one_hand_down = False  # Reset one hand down state
        else:
            previous_one_hand_down = False
            previous_two_hands_down = False
